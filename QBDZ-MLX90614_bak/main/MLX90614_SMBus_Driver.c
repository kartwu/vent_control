...
void MLX90614_SMBusInit(uint8_t sda_gpio, uint8_t scl_gpio, int freq){   
    sda = sda_gpio;
    scl = scl_gpio;
    ESP_LOGI(__func__," %x ,%x, %d", sda_gpio,scl_gpio,freq);
    ...
    esp_err_t ret = i2c_param_config(I2C_NUM_0, &MLX90614_config);
    ESP_LOGI(__func__," i2s_param_config return %s", esp_err_to_name(ret));
    i2c_driver_install(I2C_NUM_0, I2C_MODE_MASTER, I2C_MASTER_RX_BUF_DISABLE, I2C_MASTER_TX_BUF_DISABLE, 0);
    ESP_LOGI(__func__," i2s_driver install return %s", esp_err_to_name(ret));
