...
struct _S_SGP_DATA {
    uint16_t tVoC;      //修改int_t 为uint16_t ，因为datasheet范围是60000 ppb
    uint16_t eCO2;      //修改int_t 为uint16_t ，因为datasheet范围是400~60000 ppm
    int16_t H2;
    int16_t Ethanol;
    uint16_t tVoC_baseline; //修改int_t 为uint16_t ，否则该参数在LOGI中输出时，位数多了4位，是 “FFFFABCD”，不知道为什么。
    uint16_t eCO2_baseline; //修改int_t 为uint16_t ，否则该参数在LOGI中输出时，位数多了4位，是 “FFFFABCD”，不知道为什么。
    int16_t featureSetVer;
    uint64_t serialID;
} sgp30_Data;

static const uint16_t SGP_CMD_INIT = 0x2003;    //sgp30_iaq_init
static const uint16_t SGP_CMD_GET_MEASURE = 0x2008;     //sgp30_measure_iaq
