...
esp_err_t sgp30_Init(sgp30_param_t params) {
    if(params.using_library_i2c) {
        ...
        ESP_LOGI(TAG, "sda_io_num %d", params.sda);
        conf.sda_io_num = params.sda;
        conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
        ESP_LOGI(TAG, "scl_io_num %d", params.scl);
        conf.scl_io_num = params.scl;
        conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
        ESP_LOGI(TAG, "clk_speed %d", params.i2c_freq_hz);
    ...
    //     ESP_LOGI(TAG, "I am waiting here.\n");
    // }
    return err;
}
...
esp_err_t sgp30_IAQ_Init() {
    esp_err_t err = ESP_OK;
    if((err = sgp30_SendCommand(SGP_CMD_INIT)) != ESP_OK) {
        ESP_LOGE(TAG, "sgp30_IAQ_Init failed: %04X", err);
        return err;
    }
...
//        ESP_LOGE(TAG, "I2C Read failed 0x%04X", err);
    ...
    crc = sgp30_CalcChecksum(sgp30_Data.eCO2_baseline);  //修改sgp30_Data.eCO2 为 sgp30_Data.eCO2_baseline
    if(crc != data[2]) {
        err = SGP30_ERR_BAD_CRC;
    ...
    crc = sgp30_CalcChecksum(sgp30_Data.tVoC_baseline); //修改sgp30_Data.tVoC 为 sgp30_Data.tVoC_baseline
    if(crc != data[5]) {
        err = SGP30_ERR_BAD_CRC;
